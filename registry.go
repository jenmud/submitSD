...
func (s *Store) remove(node *ExpiryNode) error {
	s.lock.Lock()
	defer s.lock.Unlock()

	if n, ok := s.reg[node.GetUid()]; ok {
		logrus.Infof("Removing node %s", n)
...
func (s *Store) Unregister(ctx context.Context, node *Node) (*Node, error) {
	...
	if xn, ok := s.reg[node.GetUid()]; ok {
		logrus.Infof("Unregistering node %s at %s", xn, time.Now().UTC())
		xn.Expire()
		return xn.Node, nil
	}
